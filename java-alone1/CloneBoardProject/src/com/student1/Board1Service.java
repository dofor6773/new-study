...
import com.studebt1.utlis.DBManager;
...
public class Board1Service {
	...
	public void selectAllBoard() {
		...
		int countRows = 0;   // 행의 번호호호
		
		try {
			pstmt = conn.prepareStatement(selectSql);
			rs = pstmt.executeQuery();
			
			while(rs.next()) {   // sql시행한 데이터들의 1행을 가져옴(데이터가 있으면 true, 없으면 false)
				countRows++;
...
//				);
	...
	public void selectOneBoard() {
		...
		try {
			...
			rs = pstmt.executeQuery();   // slect 실행(count)
            ...
            if (rs.next()) {  // sql실행한 데이터들의 1행은 가져옴(데이터가 있으면 true, 없으면 false)
			...
			pstmt.setInt(1, boardNo);
			rs = pstmt.executeQuery();  // update 실행(detail) 
		} catch(SQLException se) {
